...
import { AddBuildPCConfigModalComponent } from './components/add-build-pc-configs-modal.component';
import { BuildPcConfigsService } from '../_services/build-pc-configs.service';

@Component({
  selector: 'app-build-pc-configs',
  templateUrl: './build-pc-configs.component.html',
  styleUrls: [],
})
export class BuildPCConfigsComponent
  implements
  OnInit,
  OnDestroy,
 // IDeleteAction,
  ...
  constructor(
    ...
    public buildPcService : BuildPcConfigsService
  ) { }
  ...
  ngOnInit(): void {
   // this.getListCategoryParent();
    this.filterForm();
    this.searchForm();
    this.buildPcService.fetch();
    // const sb = this.tradeService.isLoading$.subscribe(res => this.isLoading = res);
    // this.subscriptions.push(sb);
    this.grouping = this.buildPcService.grouping;
    this.paginator = this.buildPcService.paginator;
  ...
  filter() {
    ...
    // }
    this.buildPcService.patchState({ filter });
  }
  ...
  */
  ...
  search(searchTerm: string) {
    this.buildPcService.patchState({ searchTerm });
  }
  ...
  sort(column: string) {
    ...
    this.buildPcService.patchState({ sorting });
  }

  // pagination
  paginate(paginator: PaginatorState) {
    this.buildPcService.patchState({ paginator });
  }
  // actions
  ...
  edit(id: number, search: string, name: string) {
    const modalRef = this.modalService.open(AddBuildPCConfigModalComponent, {size: 'xl'});
    modalRef.componentInstance.id = id;
